...
<div class="container mt-4">
    <div class="card shadow-lg border-0 rounded-4">
        ...
        <div class="card-body" style="background-color: #343a40; color: #d4af37;">
            <form method="POST" action="{{ route('carros.alterar') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="row mb-3">
                        ...
                        <div class="d-flex">
                            <select id="marca_id" name="marca_id" class="form-select form-control bg-dark text-light border-warning me-2" >
                                ...
                                @foreach ($marcas as $marca)
                                    <option value="{{ $marca->id }}" {{ $carros->marca_id == $marca->id ? 'selected' : '' }}>
                                        {{ $marca->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="d-flex">
                            <select id="cor_id" name="cor_id" class="form-select form-control bg-dark text-light border-warning me-2" >
                                ...
                                @foreach ($cores as $cor)
                                    <option value="{{ $cor->id }}" {{ $carros->cor_id == $cor->id ? 'selected' : '' }}>
                                        {{ $cor->nome }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-2">
                        ...
                        <input type="number" class="form-control bg-dark text-light border-warning" 
                               id="ano" name="ano" placeholder="2022" value="{{ $carros->ano }}"  />
                    </div>

                    <div class="col-md-2">
                        <label for="quilometragem" class="form-label">Quilometragem</label>
                        <input type="number" class="form-control bg-dark text-light border-warning" 
                               id="quilometragem" name="quilometragem" placeholder="Ex: 45000" value="{{ $carros->quilometragem }}"  />
                    </div>
                ...
                <div class="row mb-1">
                    ...
                    <div class="col-md-2">
                        ...
                        <select id="status" name="status"
                                class="form-select form-control bg-dark text-light border-warning me-2">
                            <option value="">Selecione...</option>
                            <option value="Novo" {{ old('status', $carros->status) == 'Novo' ? 'selected' : '' }}>Novo</option>
                            <option value="Seminovo" {{ old('status', $carros->status) == 'Seminovo' ? 'selected' : '' }}>Seminovo</option>
                        </select>
                    </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control bg-dark text-light border-warning" 
                              ...
                              placeholder="Descrição detalhada do veículo...">{{ $carros->descricao }}</textarea>
                </div>
...
window.addEventListener('DOMContentLoaded', function () {
    ...
    let selectedModelo = "{{ $carros->modelo_id }}";
    let selectModelos = document.getElementById('modelo_id');
